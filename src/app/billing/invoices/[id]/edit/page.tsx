...
export default function EditInvoicePage({ params }: { params: { id: string } }) {
  ...
  const [invoiceNumber, setInvoiceNumber] = useState('EHC-2025-014');
  const [invoiceDate, setInvoiceDate] = useState('22-06-2025');
  const [selectedPatient, setSelectedPatient] = useState('1'); // Robert Wilson
  const [paymentStatus, setPaymentStatus] = useState('Partial');
  const [overallDiscount, setOverallDiscount] = useState(0);
  const [sgstRate, setSgstRate] = useState(9);
  const [cgstRate, setCgstRate] = useState(9);
  const [notes, setNotes] = useState('AI-powered hearing aid with health monitoring features.');
  const [warrantyInfo, setWarrantyInfo] = useState('Payment is due within 30 days of invoice date. All services provided are subject to professional terms. Warranty terms apply as per individual service agreements.');
  const [items, setItems] = useState<InvoiceItem[]>([
    {
      id: 1,
      serviceId: 5,
      serviceName: 'Starkey Livio AI 2400 Hearing Aid',
      description: 'AI-powered hearing aid with health monitoring.',
      qty: 1,
      unitCost: 69000,
      discount: 4000,
    ...
    },
    {
      id: 3,
      serviceId: 7,
      serviceName: 'Health Monitoring Setup',
      description: 'AI health feature configuration and training.',
      qty: 1,
      unitCost: 1500,
      discount: 0,
  ...
  const handleSave = () => {
    // TODO: Save invoice changes
    console.log('Saving invoice changes...');
    window.location.href = `/billing/invoices/${params.id}`;
  };
